...
def dashboards():
    ...
    # print(df_time_data)
    # print(df_time_data.query('kid == "Calvin"')['value1'].sum())
    ...
    for book, kid in zip(['Algebra_1_2', 'Math_7_6'], ['Calvin', 'Samuel']):
        ...
        # restrict df_o to only those he's done.
        performance = pd.DataFrame(list(db_performance[book].find({'kid': kid}))[2:])
        if book not in ['Math_5_4', 'Math_6_5', 'Math_7_6', 'Math_8_7']:
            performance = performance.loc[~performance['end_chapter'].str.contains('test', na=False)]
